    ...
    <header className="sticky top-0 z-50 bg-white border-b border-gray-200 shadow-sm">
      ...
      {searchOpen && (
        <div className="lg:hidden border-t border-gray-100 bg-white px-4 py-3">
          <form onSubmit={(e) => { handleSearch(e); setSearchOpen(false); }} role="search">
            <label htmlFor="mobile-search-expand" className="sr-only">Search products</label>
            <div className="relative">
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search by name, size, colour…"
                className="w-full bg-gray-50 border border-gray-200 rounded-full pl-9 pr-10 py-2.5 text-sm focus:outline-none focus:border-[#1a6b2f]"
                autoFocus
              />
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
              </svg>
              <button type="button" onClick={() => setSearchOpen(false)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" aria-label="Close search">
      ...
      {menuOpen && (
        <div className="lg:hidden border-t border-gray-200 bg-white px-4 py-4 max-h-[80vh] overflow-y-auto">
          ...
          <nav className="flex flex-col gap-1" aria-label="Mobile navigation">
            <MobileCategoryAccordion categories={dynamicCategories} onNavigate={() => setMenuOpen(false)} />
...
function MobileCategoryAccordion({ categories, onNavigate }: { categories: Record<string, string[]>; onNavigate: () => void }) {
      ...
      {Object.entries(categories).map(([cat, subs]) => (
        <div key={cat}>
          <button
            ...
            className="w-full flex items-center justify-between py-2.5 text-sm font-semibold text-gray-800"
            aria-expanded={expanded === cat}
          >
            {cat}
            <svg
              className={`w-4 h-4 text-gray-400 transition-transform ${expanded === cat ? "rotate-180" : ""}`}
          ...
          {expanded === cat && (
            <div className="pl-3 pb-2 space-y-1">
              <Link
                href={`/shop?category=${encodeURIComponent(cat)}`}
                className="block py-1.5 text-sm font-medium text-[#1a6b2f]"
                onClick={onNavigate}
              >
                View all {cat}
              </Link>
              {subs.map((sub) => (
                <Link
                  key={sub}
                  href={`/shop?category=${encodeURIComponent(cat)}&sub=${encodeURIComponent(sub)}`}
                  className="block py-1.5 text-sm text-gray-600 hover:text-[#1a6b2f]"
                  onClick={onNavigate}
                >
                  {sub}
                </Link>
